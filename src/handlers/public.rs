...
pub async fn handle_add(bot: Bot, chat_id: ChatId, wallet: String, ctx: &AppContext) {
    ...
    if w.is_empty() || !w.starts_with("kaspa:") {
        return;
    }
    ...
    ctx.state
        ...
        .insert(chat_id.0);
    let _ = bot
        .send_message(chat_id, format!("✅ <b>Tracked:</b>\n<code>{}</code>", w))
        .parse_mode(teloxide::types::ParseMode::Html)
        .await;
}
...
pub async fn handle_remove(bot: Bot, chat_id: ChatId, wallet: String, ctx: &AppContext) {
    ...
    if let Some(mut u) = ctx.state.get_mut(&w) {
        u.remove(&chat_id.0);
    }
    let _ = bot
        .send_message(chat_id, "🗑️ <b>Removed.</b>")
        .parse_mode(teloxide::types::ParseMode::Html)
        .await;
}
...
pub async fn handle_list(bot: Bot, chat_id: ChatId, ctx: &AppContext) {
    ...
    for e in ctx.state.iter().filter(|e| e.value().contains(&chat_id.0)) {
        tr.push_str(&format!("• <code>{}</code>\n", e.key()));
    }
    let text = if tr.is_empty() {
        "📂 <b>No wallets.</b>".to_string()
    } else {
        format!("📂 <b>Tracked:</b>\n{}", tr)
    };
    let _ = bot
