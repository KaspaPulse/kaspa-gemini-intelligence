...
pub async fn handle_start(bot: Bot, chat_id: ChatId, user_id: i64, ctx: &AppContext) {
    let markup = if user_id == ctx.admin_id { crate::kaspa_features::admin_menu_markup() } else { crate::kaspa_features::main_menu_markup() };
    ...
    let _ = bot.send_message(chat_id, help_text).parse_mode(teloxide::types::ParseMode::Html).reply_markup(markup).await;
}
...
pub async fn handle_help(bot: Bot, chat_id: ChatId) {
    ...
    let _ = bot.send_message(chat_id, help_text).parse_mode(teloxide::types::ParseMode::Html).await;
}
...
pub async fn handle_add(bot: Bot, chat_id: ChatId, wallet: String, ctx: &AppContext) {
    ...
    if wallet.is_empty() || !wallet.starts_with("kaspa:") {
        let _ = bot.send_message(chat_id, "⚠️ <b>Invalid Format.</b>\nUse: <code>/add kaspa:q...</code>").parse_mode(teloxide::types::ParseMode::Html).await;
        return;
    }
    crate::state::add_wallet_to_db(&ctx.pool, &wallet, chat_id.0).await;
    ctx.state.entry(wallet.clone()).or_insert_with(HashSet::new).insert(chat_id.0);
    
    let is_local = is_local_node();
    let sync_status_msg = if is_local { "\n\n🔄 <i>Historical sync started from pruning point...</i>" } else { "\n\n⚠️ <i>Live tracking active. (Historical sync disabled on public nodes)</i>" };
    let _ = bot.send_message(chat_id, format!("✅ <b>Wallet Added!</b>\n<code>{}</code> is now monitored.{}", wallet, sync_status_msg)).parse_mode(teloxide::types::ParseMode::Html).await;
...
pub async fn handle_remove(bot: Bot, chat_id: ChatId, wallet: String, ctx: &AppContext) {
    ...
    if let Some(mut users) = ctx.state.get_mut(&wallet) { users.remove(&chat_id.0); }
    let _ = bot.send_message(chat_id, "🗑️ <b>Wallet Removed.</b>").parse_mode(teloxide::types::ParseMode::Html).await;
}

pub async fn handle_list(bot: Bot, chat_id: ChatId, ctx: &AppContext) {
    let mut tracked = String::new();
    for e in ctx.state.iter().filter(|e| e.value().contains(&chat_id.0)) { tracked.push_str(&format!("• <code>{}</code>\n", e.key())); }
    let text = if tracked.is_empty() { "📂 <b>No wallets tracked.</b>".to_string() } else { format!("📂 <b>Tracked Wallets:</b>\n{}", tracked) };
    let _ = bot.send_message(chat_id, text).parse_mode(teloxide::types::ParseMode::Html).await;
}

pub async fn handle_balance(bot: Bot, chat_id: ChatId, ctx: &AppContext, current_utc_time: String, edit_msg_id: Option<teloxide::types::MessageId>) {
    let mut total = 0.0;
    let mut text = format!("💰 <b>Wallet Analysis & Live Balance</b>\n⏱️ <code>{}</code>\n━━━━━━━━━━━━━━━━━━\n", current_utc_time);
    let tracked_wallets: Vec<String> = ctx.state.iter().filter(|e| e.value().contains(&chat_id.0)).map(|e| e.key().clone()).collect();
    ...
    let _ = send_or_edit_log(&bot, chat_id, edit_msg_id, text, Some(refresh_markup("refresh_balance"))).await;
}

pub async fn handle_blocks(bot: Bot, chat_id: ChatId, ctx: &AppContext, current_utc_time: String, edit_msg_id: Option<teloxide::types::MessageId>) {
    let tracked: Vec<String> = ctx.state.iter().filter(|e| e.value().contains(&chat_id.0)).map(|e| e.key().clone()).collect();
    if tracked.is_empty() {
        let _ = send_or_edit_log(&bot, chat_id, edit_msg_id, "⚠️ <b>No wallets tracked.</b>".to_string(), None).await; return;
    }
    let mut text = format!("🧱 <b>Mined Blocks Analysis</b>\n⏱️ <code>{}</code>\n━━━━━━━━━━━━━━━━━━\n", current_utc_time);
    ...
    for w in tracked {
        if let Ok((total_blocks, total_kas)) = crate::state::get_lifetime_stats(&ctx.pool, &w).await {
            ...
            text.push_str(&format!("💼 <b>{}</b>\n├ <b>Lifetime Blocks:</b> {}\n├ <b>Lifetime Value:</b> {:.8} KAS\n", format_short_wallet(&w), total_blocks, total_kas));
            ...
            if let Ok(records) = daily_records {
                if !records.is_empty() {
                    ...
                    for (date, count, amount) in records { text.push_str(&format!("│  ▪ <code>{}</code>: {} Blks ({:.2} KAS)\n", date, count, amount)); }
                }
            }
            text.push('\n');
        }
    }
    text.push_str(&format!("━━━━━━━━━━━━━━━━━━\n🏆 <b>Total Blocks:</b> {}\n💎 <b>Total Value:</b> {:.8} KAS", global_blocks, global_rewards));
    let _ = send_or_edit_log(&bot, chat_id, edit_msg_id, text, Some(refresh_markup("refresh_blocks"))).await;
}

pub async fn handle_miner(bot: Bot, chat_id: ChatId, ctx: &AppContext, current_utc_time: String, edit_msg_id: Option<teloxide::types::MessageId>) {
    let tracked: Vec<String> = ctx.state.iter().filter(|e| e.value().contains(&chat_id.0)).map(|e| e.key().clone()).collect();
    if tracked.is_empty() { return; }
    let mut text = format!("⛏️ <b>Solo-Miner Hashrate</b>\n⏱️ <code>{}</code>\n━━━━━━━━━━━━━━━━━━\n", current_utc_time);
    if let Ok(dag_info) = ctx.rpc.get_block_dag_info().await {
        if let Ok(net_hashrate) = ctx.rpc.estimate_network_hashes_per_second(1000, None).await {
            ...
            for w in tracked {
                ...
                let mut live_1h = 0; let mut live_24h = 0;
                if let Ok(addr) = Address::try_from(w.as_str()) {
                    if let Ok(utxos) = ctx.rpc.get_utxos_by_addresses(vec![addr]).await {
                        for u in utxos.into_iter().filter(|u| u.utxo_entry.is_coinbase) {
                            let age = dag_info.virtual_daa_score.saturating_sub(u.utxo_entry.block_daa_score);
                            if age <= 3600 { live_1h += 1; }
                            if age <= 86400 { live_24h += 1; }
                        }
                    }
                }
            ...
            text.push_str(&format!("━━━━━━━━━━━━━━━━━━\n🌐 <b>Global Network Hashrate:</b> {}", crate::kaspa_features::format_hashrate(net_hashrate)));
        }
    }
    let _ = send_or_edit_log(&bot, chat_id, edit_msg_id, text, Some(refresh_markup("refresh_miner"))).await;
}

pub async fn handle_network(bot: Bot, chat_id: ChatId, ctx: &AppContext, current_utc_time: String, edit_msg_id: Option<teloxide::types::MessageId>) {
    let mut text = String::from("🛠️ <b>Node Health & Network</b>\n");
    if let Ok(info) = ctx.rpc.get_server_info().await { text.push_str(&format!("├ <b>Version:</b> {} | <b>Net:</b> {}\n├ <b>UTXO Index:</b> {}\n", info.server_version, info.network_id, if info.has_utxo_index { "Enabled ✅" } else { "Disabled ❌" })); }
    if let Ok(peers) = ctx.rpc.get_connected_peer_info().await { text.push_str(&format!("├ <b>Connected Peers:</b> {}\n", peers.peer_info.len())); }
    if let Ok(sync) = ctx.rpc.get_sync_status().await { text.push_str(&format!("└ <b>Sync Status:</b> {}\n\n", if sync { "100% Synced ✅" } else { "Syncing ⚠️" })); }
    text.push_str("📊 <b>GHOSTDAG Consensus</b>\n");
    if let Ok(dag) = ctx.rpc.get_block_dag_info().await { text.push_str(&format!("├ <b>Total Blocks:</b> {}\n├ <b>DAA Score:</b> {}\n├ <b>Difficulty:</b> {}\n", f_num(dag.block_count as f64), dag.virtual_daa_score, crate::kaspa_features::format_difficulty(dag.difficulty))); }
    if let Ok(hashrate) = ctx.rpc.estimate_network_hashes_per_second(1000, None).await { text.push_str(&format!("├ <b>Hashrate:</b> {}\n", crate::kaspa_features::format_hashrate(hashrate as f64))); }
    ...
    let _ = send_or_edit_log(&bot, chat_id, edit_msg_id, text, Some(refresh_markup("refresh_network"))).await;
}

pub async fn handle_dag(bot: Bot, chat_id: ChatId, ctx: &AppContext, current_utc_time: String, edit_msg_id: Option<teloxide::types::MessageId>) {
    if let Ok(info) = ctx.rpc.get_block_dag_info().await {
        ...
        let _ = send_or_edit_log(&bot, chat_id, edit_msg_id, text, Some(refresh_markup("refresh_dag"))).await;
    }
}

pub async fn handle_price(bot: Bot, chat_id: ChatId, ctx: &AppContext, current_utc_time: String, edit_msg_id: Option<teloxide::types::MessageId>) {
    ...
    let _ = send_or_edit_log(&bot, chat_id, edit_msg_id, text, Some(refresh_markup("refresh_price"))).await;
}

pub async fn handle_market(bot: Bot, chat_id: ChatId, ctx: &AppContext, current_utc_time: String, edit_msg_id: Option<teloxide::types::MessageId>) {
    ...
    let _ = send_or_edit_log(&bot, chat_id, edit_msg_id, text, Some(refresh_markup("refresh_market"))).await;
}

pub async fn handle_supply(bot: Bot, chat_id: ChatId, ctx: &AppContext, current_utc_time: String, edit_msg_id: Option<teloxide::types::MessageId>) {
    if let Ok(supply) = ctx.rpc.get_coin_supply().await {
        let circ = supply.circulating_sompi as f64 / 1e8; let max = supply.max_sompi as f64 / 1e8;
        ...
        let _ = send_or_edit_log(&bot, chat_id, edit_msg_id, text, Some(refresh_markup("refresh_supply"))).await;
    }
}

pub async fn handle_fees(bot: Bot, chat_id: ChatId, current_utc_time: String, edit_msg_id: Option<teloxide::types::MessageId>) {
    if let Ok(r) = reqwest::get("https://api.kaspa.org/info/fee-estimate").await {
        if let Ok(j) = r.json::<serde_json::Value>().await {
            ...
            let _ = send_or_edit_log(&bot, chat_id, edit_msg_id, text, Some(refresh_markup("refresh_fees"))).await;
        }
    }
}
