...
impl PostgresRepository {
    ...
    pub async fn update_sync_checkpoint(&self, _address: &str, _daa_score: u64) -> Result<(), AppError> {
        Ok(())
    }
    ...
    pub async fn add_to_knowledge_base(&self, title: &str, link: &str, content: &str, source: &str) -> Result<bool, AppError> {
    ...
    pub async fn get_unindexed_knowledge(&self, limit: i64) -> Result<Vec<(i32, String)>, AppError> {
        let rows = sqlx::query!(
    ...
    pub async fn update_knowledge_embedding(&self, id: i32, embedding: Vec<f32>) -> Result<(), AppError> {
