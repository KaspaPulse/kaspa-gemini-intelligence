...
pub fn admin_menu_markup() -> InlineKeyboardMarkup {
    let rows = vec![
        ...
        vec![InlineKeyboardButton::callback("❤️ Support Developer", "cmd_donate")], // Re-added here

        // Section 2: Visual Separator
        vec![InlineKeyboardButton::callback("─── ⚙️ ADMIN TERMINAL ⚙️ ───", "none")],
        ...
        vec![InlineKeyboardButton::callback("📢 Broadcast Message", "cmd_broadcast")],
    ];
...
pub fn format_difficulty(val: f64) -> String {
    ...
    if val >= 1e15 { format!("{:.2} P", val / 1e15) } 
    else if val >= 1e12 { format!("{:.2} T", val / 1e12) } 
    else if val >= 1e9 { format!("{:.2} G", val / 1e9) } 
    else { format!("{:.2}", val) }
}
