...
impl LocalAiEngine {
    pub fn new() -> anyhow::Result<Self> {
        ...
        let chat_model =
            std::env::var("AI_CHAT_MODEL").unwrap_or_else(|_| "llama3-8b-8192".to_string());
        let audio_model =
        ...
        if let Some(bytes) = audio_bytes {
            ...
            if status.is_success() {
                ...
                return Err(anyhow::anyhow!(
                    "Audio transcription failed. Ensure the provider supports Whisper."
                ));
            }
        }
...
{}",
        ...
        Err(anyhow::anyhow!(
            "AI servers are currently overloaded. Details: {}",
            last_error
        ))
    }
}
